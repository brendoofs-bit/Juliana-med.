  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden animate-fade-in-up flex flex-col md:flex-row max-h-[90vh] md:max-h-[85vh]">
        ...
        <div className="w-full md:w-5/12 bg-white p-4 md:p-8 flex items-center justify-center border-b md:border-b-0 md:border-r border-gray-100 relative shrink-0 h-56 md:h-auto overflow-hidden group">
        ...
        {/* Seção Conteúdo */}
        <div className="w-full md:w-7/12 flex flex-col h-full overflow-hidden bg-white">
            {/* Header (Fixo) */}
            <div className="p-4 md:p-6 border-b border-gray-100 flex justify-between items-start bg-white shrink-0 z-10 relative">
              <div className="pr-8">
                <h3 className="text-xl md:text-2xl font-extrabold text-medical-900 leading-tight">{product.name}</h3>
                <p className="text-sm text-gray-500 mt-1 font-medium">{product.description}</p>
                {product.tags && (
            ...
            {/* Descrição (Scrollável com scrollbar customizada) */}
            <div className="p-4 md:p-8 overflow-y-auto flex-1 custom-scrollbar text-sm text-gray-600 leading-relaxed bg-white">
              ...
              {/* O CTA já vem no HTML do benefits, mas adicionamos um botão nativo extra caso o HTML falhe ou para reforço */}
            ...
            <div className="p-4 md:p-6 border-t border-gray-100 bg-gray-50 shrink-0 z-10 relative shadow-[0_-4px_10px_rgba(0,0,0,0.03)] flex flex-col gap-2">
               <button 
                 onClick={handleWhatsAppRedirect}
